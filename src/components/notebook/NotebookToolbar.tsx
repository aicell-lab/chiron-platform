...
const ToolbarDropdown: React.FC<ToolbarDropdownProps> = ({
  ...
  const [tempTitle, setTempTitle] = useState(metadata.title || 'Chiron Lab');

  const handleTitleClick = () => {
    setIsEditingTitle(true);
    setTempTitle(metadata.title || 'Chiron Lab');
  };

  const handleTitleSave = () => {
    onMetadataChange({
      ...metadata,
      title: tempTitle || 'Chiron Lab',
    ...
    <div className="absolute right-0 top-full mt-1 w-64 bg-white rounded-md shadow-lg z-50 py-1 border border-gray-200">
      ...
      <div className="md:hidden px-2 py-2 border-b border-gray-200">
        {isEditingTitle ? (
          ...
          >
            <div className="font-medium truncate">{metadata.title || 'Chiron Lab'}</div>
