...
// The chiron-models collection root is read on every Hub / Train page load
// and is the same code path that surfaces an outage to the user, so probing
// it gives a faithful "are we back" signal.
const HEALTH_URL = 'https://hypha.aicell.io/chiron-platform/artifacts/chiron-models';
 ...
 * tiny probe loop against the chiron-models collection endpoint that the
 * Hub and Train pages talk to; on the first 2xx the global flag clears and
 * the banner unmounts itself.
 *
    ...
    >
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 py-2 flex items-center gap-3 text-sm">
        ...
        <span className="text-amber-900 flex-1 min-w-0">
          <strong className="font-semibold">Chiron Platform services are temporarily unreachable.</strong>{' '}
          <span className="text-amber-900/85">
            The Model Hub, training, and most interactive features pause while the backend restarts.{' '}
            {isProbing ? (
