...
- `transport: "websocket" | "webrtc"` — which transport carries the round protocol (`start_fit`, `get_fit_status`, `get_fit_result`, `start_evaluate`, `get_evaluate_status`, `get_parameters`). Default `websocket`. The weights themselves ride `start_fit`, `get_fit_result` and `get_parameters`. Control-plane calls always use the WebSocket regardless, because they are small and rely on Hypha's routing.
