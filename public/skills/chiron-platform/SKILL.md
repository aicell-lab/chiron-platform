...
**Tabula, scGPT and Geneformer are the models to launch.** A worker's container image decides which model it can train: it carries that model's dependencies and hosts that model's trainer, and no other. Chiron guarantees those three today. scFoundation is still being brought online and appears on [chiron.aicell.io/#/models](https://chiron.aicell.io/#/models) as an architecture that is coming. The setup wizard lists it but will not start a worker on it, so do not generate a launch command for it either. If the user asks for scFoundation, say it is coming and offer one of the three.
...
| Geneformer | `ghcr.io/aicell-lab/chiron-geneformer:<version>` | `chiron-platform/geneformer-trainer` | Supported |
| scFoundation | `ghcr.io/aicell-lab/chiron-scfoundation:<version>` | `chiron-platform/scfoundation-trainer` | Coming soon |

**scGPT and Geneformer need a gene panel they can name.** Tabula reads a prepared store's binned layer by position, so any prepared dataset will do. The other two identify genes rather than positions, so each needs its own identifier column in the store alongside the binned layer: scGPT matches on HGNC symbol in `var/feature_name`, Geneformer on Ensembl gene ID in `var/feature_id`, looked up directly in its own vocabulary with no symbol mapping in between. A store prepared without the column its model needs deploys fine and then fails inside the trainer with "No usable cells found". Check the dataset before launching the worker on it rather than after.
...
- **Compute headroom**: `nproc` for CPU cores and `free -h` (Linux) or equivalent for RAM. The wizard defaults to 4 CPU and 1 GPU. RAM depends on the model, because Ray admits an application only if its declared memory fits the head node's budget and a worker holds the manager (1 GB), the orchestrator (8 GB) and the trainer at once: **30 GB for Tabula and for scGPT, 40 GB for Geneformer**, and later 48 GB for scFoundation. A worker started with less comes up healthy and then refuses its trainer with `Insufficient resources`. The wizard fills the right figure in when you pick the model.
