...
**scGPT needs a gene panel it can name.** Tabula reads a prepared store's binned layer by position, so any prepared dataset will do. scGPT matches genes by HGNC symbol instead, so it needs `var/feature_name` in the store alongside the binned layer. A store prepared without that column deploys fine and then fails inside the trainer with "No usable cells found". Check the dataset before launching an scGPT worker on it rather than after.
...
- **Compute headroom**: `nproc` for CPU cores and `free -h` (Linux) or equivalent for RAM. The wizard defaults to 4 CPU and 1 GPU. RAM depends on the model, because Ray admits an application only if its declared memory fits the head node's budget and a worker holds the manager (1 GB), the orchestrator (8 GB) and the trainer at once: **30 GB for Tabula and for scGPT**, and later 40 GB for Geneformer and 48 GB for scFoundation. A worker started with less comes up healthy and then refuses its trainer with `Insufficient resources`. The wizard fills the right figure in when you pick the model.
