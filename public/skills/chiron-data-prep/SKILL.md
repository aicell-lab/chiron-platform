...
3. **Ranks genes by over-dispersion** (`variance / mean`) on the QC-passing submatrix only. Genes that fail gene QC get score 0 and rank last so the trainer's top-K selection naturally excludes them. Written into the zarr's `var/` group:
   - `var/tabula_hvg_rank` — int32, rank 0 = most variable
   - `var/tabula_hvg_score` — float32, the raw score
   - root attrs: `tabula_hvg_method`, `tabula_hvg_version`, `tabula_hvg_histogram_counts` (length-20 log-spaced histogram so the dataset card can render a sparkline without sending 30k floats), `tabula_hvg_histogram_edges_log10`
4. **Bins expression values per cell** into 50 equal-frequency quantile bins. Zero values stay at bin 0, non-zero values are ranked per row and assigned bins 1..50. Written as a uint8 layer:
   - `layers/tabula_binned` — uint8, shape `(n_cells, n_vars)`
   - root attrs: `tabula_binning_version`, `tabula_binning_n_bins`
5. **Computes a 2D UMAP embedding** of the cells using streaming IncrementalPCA followed by UMAP on a subsample:
   ...
   - 2D coordinates are written as `obsm/tabula_umap_coords` (float32) and `obsm/tabula_umap_indices` (int32, only present if subsampled). Root attrs: `tabula_umap_method`, `tabula_umap_version`, `tabula_umap_n_sampled`.
...
- Filters cells by `obs/tabula_qc_pass` when present, contributing only QC-passing rows to the federated training mix.
- Reads expression values from `layers/tabula_binned` when present, falling back to `layers/X_binned` (older upstream pipelines) and finally raw `X` (legacy zarrs).
- If `n_vars ≤ in_feature`, keeps all genes and pads on the right with `padding_id`.
- If `n_vars > in_feature` and `var/tabula_hvg_rank` is present, keeps the `in_feature` genes with the lowest rank (the most variable).
