...
The platform guarantees **Tabula**, **scGPT** and **Geneformer**. The scFoundation image is built and published and its trainer artifact exists, but the setup wizard will not start a worker on it and its architecture card on `#/models` is marked coming soon. Models are brought online one at a time, each as its own pair of PRs, and scFoundation is next. A maintainer can still run it by hand.

scGPT reads a prepared store by gene symbol, so it needs `var/feature_name` alongside `layers/chiron_binned`. Geneformer reads it by Ensembl accession and needs `var/feature_id` instead, looked up straight in its own 20,275-token vocabulary with no symbol mapping in between. Tabula reads the binned layer positionally and needs neither. A store prepared without the column its model wants deploys and then fails inside the trainer with `No usable cells found`, so check the dataset before pointing a worker at it. The platform compares the columns a worker reports against the model it runs and says so in the launch dialog, but only for a manager new enough to report them.
...
| Geneformer | `ghcr.io/aicell-lab/chiron-geneformer:<version>` | `chiron-platform/geneformer-trainer` | Supported | 8 (about 13 GB on a 24 GB RTX 3090, 4 is about 5 GB) | 24 GiB | 40 |
| scFoundation | `ghcr.io/aicell-lab/chiron-scfoundation:<version>` | `chiron-platform/scfoundation-trainer` | Coming soon | 8 | 32 GiB | 48 |

Tabula and Geneformer have measured memory curves on a 24 GB RTX 3090. scGPT's and scFoundation's numbers are only the sizes that ran in a probe, so their entries in `chironModels.ts` carry `gb: 0`, which the launch dialog renders as a validated size with no memory figure rather than a misleading zero.

Geneformer's figure is the one to be careful with. Its memory tracks the cell rather than the gene panel, because a rank-value encoding is only as long as the cell has expressed genes. The demo blood store tokenises to a median of 228 genes and runs batch 16 without trouble, while pbmc3k tokenises to a median of 789 and runs out of memory there. The 8 above is measured against the deeper of the two, and it is what the launch dialog prefills for a Geneformer worker.
